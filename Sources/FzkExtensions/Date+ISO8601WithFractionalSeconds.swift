...
public extension Date {
	/// Parses the passed-in value according to the
	/// [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html)
	/// and [RFC 3339](https://www.ietf.org/rfc/rfc3339) standards.
	///
	...
	init?(iso8601: String) {
		guard let d = ISO8601DateFormatter().date(from: iso8601)
			?? Formatter.iso8601WithFractionalSeconds.date(from: iso8601)
		else { return nil }
		self = d
	}
...
public extension JSONEncoder.DateEncodingStrategy {
	/// A strategy that formats dates according to the ISO 8601 and RFC 3339 standards, including support for fractional seconds.
...
public extension JSONDecoder.DateDecodingStrategy {
	/// A strategy that formats dates according to the ISO 8601 standard, including support for fractional seconds.
	static let iso8601WithFractionalSeconds = custom {
